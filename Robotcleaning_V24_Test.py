...
##################################################################################    
    ...
    def updateHeatmap(self,count):
        ...
        #    self.PositionWalkmanMatrix(robs)
        Dir='Testheatmap2/Test_heatmap_ore 20_22/MyHeatmap'+str(count)+'.csv'
        #print("count",count)
    ...
    while MyEpTest<11:  # Run until solved
        ...
        for timestep in range(1, max_steps_per_episode+1):
           ...
           # print(N_Walkman)
            ...
            if (timestep%update_period==0) and (countUpdatePeriod<6):
            ...
            #print("terza comunicazione:invio l'env.Data aggiornato al processo",type(env.Data),np.shape(env.Data))
                ...
                #env.Data[:, :, 0] = gaussian_filter(env.Data[:, :, 0], sigma=0.9, truncate=4.0)
                #super_threshold_indices2 = env.Data[:,:,0] < 1e-3
                #env.Data[super_threshold_indices2,0] = 0
        ...
        running_rewardSTD = np.std(StepReward_history[100:STEPS_PER_EPISODE]) # Standard deviation del reward
