...
class BlobEnv:
    ...
    def Create_Walkman(self,Walkman,N_Walkman_rand):
        #print("sono in create walkman",len(Walkman))
        #+print("N_Walkman_rand",N_Walkman_rand)
        self.N_Walkman=N_Walkman_rand
        self.Walkman=Walkman
        ...
        for robs in range(self.N_Walkman):
            ...
            self.Walkman[robs].my_Agent_action_update(randx,randy)
            #print("ho chiamato my_Agent_action_update ",robs, randx,randy)

        
            

    def reset(self,Walkman,N_Walkman_rand):
        #print("reset BlobEnv")
...
#### initialize the data matrix with dummy values only for testing
    ...
    def PositionWalkmanMatrix(self,robs):
        #Updating the heatmap with the new position of the human obstacle
        #print("PositionWalkmanMatrix")
        
        for i in range(-CLUSTER_RAY, +CLUSTER_RAY):
            if self.Walkman[robs].positionRobX[-1] + i >= N_totalX or self.Walkman[robs].positionRobX[-1] + i < 0:
                continue

            for j in range(-CLUSTER_RAY, +CLUSTER_RAY):
                if self.Walkman[robs].positionRobY[-1] + j >= N_totalY or self.Walkman[robs].positionRobY[-1] + j < 0:
                    continue

                self.Data[self.Walkman[robs].positionRobX[-1]+i, self.Walkman[robs].positionRobY[-1]+j,0] = 1
##################################################################################    
    ...
    def applyWalkman(self):
        for robs in range(self.N_Walkman):
              
            self.PositionWalkmanMatrix(robs)

    def updateHeatmap(self,timestep):
...
def main():
 ...
 #   single_robot_rewards = np.zeros((4,1))
    ...
    #TENepisode_PercentageSomma_history_DevDev = [] # only to evaluate test;
    ...
    Walkman = []
    #Walkman.clear();
    #print("sono in main, len di Walkman",len(Walkman))
    N_Walkman_rand=np.random.randint(Lowlim_N_Walk, N_Walkman)
    #print("sono in main, N_Walkman_rand",N_Walkman_rand)
    for Rango in range(N_Walkman_rand):
        Walkman.append(Walkman_class())
    #print("sono in main, len di Walkman dopo la creazione",len(Walkman),N_Walkman_rand)
    ...
    MyEpTest=0;  
    #input("press enter 1")
    while MyEpTest<51:  # Run until solved
        ...
        MyEpTest=MyEpTest+1
        env.reset(Walkman,N_Walkman_rand)
        ...
        for timestep in range(1, max_steps_per_episode+1):
            ...
            #env.updateHeatmap(timestep)

           # print(N_Walkman)
            #if timestep==np.random.randint(1, max_steps_per_episode):
            #a=np.random.randint(1,100)
            #a=np.absolute(a)
            #if a <= 30:
            # ogni 15 minuti  ricalcolo il numero di passeggeri e la nuova posizione
            ...
            if timestep%update_period==0:
                ...
                N_Walkman_rand=np.random.randint(Lowlim_N_Walk, N_Walkman)
                #print("sono nel if timestep%15 ",N_Walkman_rand)
                #print("ricalcolo N_Walkman_rand",N_Walkman_rand)
                #Walkman = []
                Walkman.clear()
                #print("Walkman si azzera?",len(Walkman))
                for Rango in range(N_Walkman_rand):
                    Walkman.append(Walkman_class())
                #print("chiamo Create_Walkman ",N_Walkman_rand)
                env.Create_Walkman(Walkman,N_Walkman_rand)

                env.applyWalkman()
            #print("chiamo updateHeatmap ",timestep)
            env.updateHeatmap(timestep)
            ...
            ###############################################################################11th block
            with open("Report"+str(N_robots)+str(N_Walkman)+".txt", "a") as f:
        ...
        reward_Somma=np.sum(StepReward_history[100:STEPS_PER_EPISODE])
        running_Somma=np.mean(episode_PercentageSomma_history[100:STEPS_PER_EPISODE])
        max_Somma_history=np.max(episode_PercentageSomma_history[100:STEPS_PER_EPISODE])
        min_Somma_history=np.min(episode_PercentageSomma_history[100:STEPS_PER_EPISODE])
        
        ######################################################fifth block 
        #testato tutto ok
        running_Variance=np.std(episode_PercentageSomma_history[100:STEPS_PER_EPISODE]) # Standard deviation della percentuale di pulizia
        running_rewardMean = np.mean(StepReward_history[100:STEPS_PER_EPISODE]) # media del reward
        running_rewardSTD = np.std(StepReward_history[100:STEPS_PER_EPISODE]) # Standard deviation del step-reward per episodio
        running_rewardMIN=np.min(StepReward_history[100:STEPS_PER_EPISODE])#min reward
        running_rewardMax=np.max(StepReward_history[100:STEPS_PER_EPISODE])#max reward
        ...
        TENrunning_rewardSTD.append(running_rewardSTD) #Standard deviation del reward per più episodi
        ...
        if running_reward > 200000:  # 100:  # Condition to consider the task solved
    ...
    Tot_running_Somma_STDSTD=np.std(TENepisode_PercentageSomma_history_Var) #STD dell'STD per più episodi
    ...
    Tot_rew_STDSTD=np.std(TENrunning_rewardSTD) #STD dell'STD per più episodi
    ...
    #TENepisode_PercentageSomma_history_DevDev.clear()
